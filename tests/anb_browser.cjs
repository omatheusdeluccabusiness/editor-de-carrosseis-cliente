...
(async () => {
  ...
  const titleBox=await page.locator('#directTitle').evaluate(el=>({height:parseFloat(el.style.height),top:parseFloat(el.style.top)}));
  const bodyBox=await page.locator('#directBody').evaluate(el=>({height:parseFloat(el.style.height),top:parseFloat(el.style.top)}));
  assert(titleBox.height<400);
  assert(bodyBox.top>titleBox.top+titleBox.height);
